  ...
  const openBox = (boxId: number) => {
    ...
    setBoxes(prevBoxes => {
      ...
      if (treasureFound || allOpened) {
        setGameEnded(true);
        if (token) {
          const result = finalScore > 0 ? 'win' : finalScore < 0 ? 'lose' : 'tie';
          recordGame(token, { score: finalScore, result }).catch(() => {});
        }
      }
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex flex-col items-center justify-center p-8">
      ...
      <div className="mb-8">
        <div className="text-2xl text-center p-4 bg-amber-200/80 backdrop-blur-sm rounded-lg shadow-lg border-2 border-amber-400">
          ...
          {gameEnded && (
            <span className={`ml-1 ${score > 0 ? 'text-green-600' : score < 0 ? 'text-red-600' : 'text-amber-700'}`}>
              ...
              >
                ...
                >
                  <img
                    ...
                    style={box.isOpen ? undefined : { cursor: `url(${keyIcon}) 16 16, pointer` }}
                  />
